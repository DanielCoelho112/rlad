...
class Platent(nn.Module):
    def __init__(self, lr, latent_size, n_actions, device, checkpoint_dir):
        super(Platent, self).__init__()

        self.device = device
        self.checkpoint_file = f"{checkpoint_dir}/weights/p_latent.pt"
        self.checkpoint_optimizer = f"{checkpoint_dir}/weights/optimizers/p_latent.pt"
        
        self.encoder = nn.Sequential(nn.Linear(latent_size + n_actions, 512),
                                     nn.LayerNorm(512),
                                     nn.ReLU(),
