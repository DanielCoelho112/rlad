...
class AllFeatTiedRegularizedEncoder(nn.Module):
    ...
    def __init__(self, device, target, checkpoint_dir, aug):
...
class AllFeatTiedRegularizedEncoder256(nn.Module):
    '''Encoder with the same regularization applied after every layer, and with the
       regularization parameter tuned only with the final layer's feature gradients.'''
    def __init__(self, latent_size, device, target, checkpoint_dir, aug):
        nn.Module.__init__(self)
        self.device = device
        self.aug = aug
...
class EncoderLSTM(nn.Module):
    ...
    def __init__(self, latent_size, device, target, checkpoint_dir, aug):
        ...
        self.lstm = nn.LSTM(input_size=latent_size, hidden_size=latent_size, num_layers=self.n_layers, bidirectional=False, batch_first=True)
...
class EncoderGRU(nn.Module):
    ...
    def __init__(self, latent_size, device, target, checkpoint_dir, aug):
        ...
        self.linear = nn.Linear(self.out_dim, latent_size)
        self.gru = nn.GRU(input_size=latent_size, hidden_size=latent_size, num_layers=self.n_layers, bidirectional=False, batch_first=True)
        self.norm = nn.Sequential(nn.LayerNorm(latent_size), nn.Tanh())
...
class EncoderV6_256(nn.Module):
    def __init__(self, latent_size, device, target, checkpoint_dir, aug, input_channels=3, pretrained=False):
...
class EncoderV6_256_GRU(nn.Module):
    def __init__(self, latent_size, device, target, checkpoint_dir, aug, input_channels=3, pretrained=False):
        super(EncoderV6_256_GRU, self).__init__()
...
class EncoderV6_280(nn.Module):
    def __init__(self, latent_size, device, target, checkpoint_dir, aug, input_channels=3, pretrained=False):
        super(EncoderV6_280, self).__init__()
...
class EncoderV7_256(nn.Module):
    
    def __init__(self, latent_size, device, target, checkpoint_dir, aug, input_channels=3, pretrained=False):    
        super(EncoderV7_256, self).__init__()
...
class EncoderV6_256_test(nn.Module):
    def __init__(self, latent_size, device, target, checkpoint_dir, aug, input_channels=3, pretrained=False):
        super(EncoderV6_256_test, self).__init__()
        ...
        self.conv1 = nn.Conv2d(input_channels, 32, 3, stride=2)
        ...
        self.conv7 = nn.Conv2d(64, 64, 3, stride=1)
        self.conv8 = nn.Conv2d(64, 64, 3, stride=1)
        self.conv9 = nn.Conv2d(64, 64, 3, stride=1)
    ...
    def forward(self, x):
        ...
        x = self.conv4(x)
        print(f'conv5: {x.size()}')
        x = self.conv5(x)
        print(f'conv6: {x.size()}')
        x = self.conv6(x)
        print(f'conv7: {x.size()}')
        x = self.conv7(x)
        
        print(f'conv8: {x.size()}')
        x = self.conv8(x)
        print(f'conv9: {x.size()}')
        x = self.conv9(x)
...
if __name__ == '__main__':
    ...
    enc = EncoderV6_256_test(latent_size=256, aug=encoder_aug, input_channels=9,
                    device=torch.device('cuda:0'), target=False, checkpoint_dir=None)

    import numpy as np 
    a = np.full((256, 9, 256, 256), 100, dtype=np.int8)
